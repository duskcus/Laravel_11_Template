@extends('layouts.default')


{{-- @SECTION FOR THE START CONTENT --}}
@section('content')


<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-4">About</h1>

    <p class="mb-4">Welcome to our store! Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed eget lorem sed libero interdum consectetur. Nunc malesuada, velit sed fermentum tincidunt, lacus metus gravida velit, sit amet consectetur libero arcu ac metus.</p>

    <p class="mb-4">Our store is committed to providing the best products and services to our customers. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed eget lorem sed libero interdum consectetur. Nunc malesuada, velit sed fermentum tincidunt, lacus metus gravida velit, sit amet consectetur libero arcu ac metus.</p>

    <p class="mb-4">If you have any questions or inquiries, feel free to <a href="/contact" class="text-blue-500 hover:underline">contact us</a>. We are here to assist you!</p>
</div>



    {{-- @ENDSECTION FOR THE END OF CONTENT --}}
    @endsection
